        ...
        if tag == 'equal':
            ...
            for line_idx in range(min(i2 - i1, j2 - j1)):
                ...
                if error_match:
                    ...
                    exec_time_pattern = re.compile(r'Execution Time:\s*([\d.]+)\s*seconds', re.IGNORECASE)

                    ctr_match = exec_time_pattern.search(ctr_line)
                    res_match = exec_time_pattern.search(res_line)

                    threshold = 0.1  # seconds

                    if ctr_match and res_match:
                        ctr_time = float(ctr_match.group(1))
                        res_time = float(res_match.group(1))
                        time_diff = abs(ctr_time - res_time)

                        if time_diff < threshold:
                            continue  # Skip reporting this small time difference

                    # Continue normal word-by-word diff
                        ...
                        if wtag == 'delete':
                            ...
                            for k in range(min(wi2 - wi1, wj2 - wj1)):
                                mismatched_details.append(
                                    f"expected '{words1[wi1 + k]}', got '{words2[wj1 + k]}'"
                                )
